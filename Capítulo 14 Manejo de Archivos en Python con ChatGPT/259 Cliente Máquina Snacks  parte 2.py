...
class MaquinaSnacks:
    ...
    def comprar_snack(self):
        id_snack = int(input('Que snack quiere comprar (id)? '))
        snack = self.servicio_snacks.get_snacks()

        snack = next((snack for snack in snack if snack.id_snack == id_snack), None)
        if snack:

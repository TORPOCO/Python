...
class EmpleadoDetail(Resource):
    # GET → Obtener un empleado    GET /api/empleados/<id>
    def get(self, id):
        empleado = Empleado.query.get_or_404(id)
        return empleado_schema.dump(empleado), 200

    # PUT → Actualizar empleado    PUT /api/empleados/<id>
    def put(self, id):
        empleado = Empleado.query.get_or_404(id)
        data = request.get_json()
        if not data:
